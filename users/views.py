...
class OrganizationView(APIView):
    ...
    """
    # permission_classes = [IsAuthenticated] # Changed to IsAdminUser
    def post(self, request):
        ...
        try:
            organization = Organization.objects.create(name=org_name, description=description)
            ...
            return Response({'organization_id': str(organization.id), 'name': organization.name, 'api_key': api_key}, status=status.HTTP_201_CREATED)
        except Exception as e:
            return Response({'error': f'Could not create organization: {str(e)}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
